...
var fdata = {
  jsonurl: '',
  apiurl: '',
  apipublieurl: 'https://www.aohuiliu.fun/friends/', //默认公共库
...
function loadStatistical(sdata){
  ...
  var loadMoreBtn = `
    ...
    <div id="cf-footer" class="cf-new-add">
     ...
     <span class="cf-data-lastupdated">友链:${sdata.friends_num} 活跃:${sdata.active_num} 文章:${sdata.article_num}</span>
    ...
    for (var i = start;i<endFor;i++){
      ...
      <div class="cf-article">
        ...
        <div class="cf-article-avatar no-lightbox flink-item-icon">
          <img class="cf-img-avatar avatar" src="${item.avatar}" onerror="this.src='${fdata.error_img}'; this.onerror = null;">
  ...
  var showHtml = `
      <div class="cf-overshow">
        <div class="cf-overshow-head">
          <img class="cf-img-avatar avatar" src="${userinfo.avatar}" onerror="this.src='${fdata.error_img}'; this.onerror = null;">
          <a class="" target="_blank" rel="noopener nofollow" href="${userinfo.link}">${userinfo.author}</a>
        </div>
        <div class="cf-overshow-content">
  `
  for (var i = 0;i<articledata.length;i++){
    // console.log(i, userinfo.article_num)
    // for (var i = 0;i<userinfo.article_num;i++)
    var item = articledata[i];
    // console.log(i, item)
    showHtml += `
      <p><a class="cf-article-title" href="${item.link}" target="_blank" rel="noopener nofollow">${item.title}</a><span>${item.created}</span></p>
    `
  }
  showHtml += '</div></div>'
...
function loadNoArticle(){
  ...
  window.scrollTo(0,document.getElementsByClassName('page-title').offsetTop)
}
// 清空本地数据
function clearLocal(){
...
function fetchShow(url){
  ...
  fetch(url)
    ...
    .then(json =>{
      ...
      noClick = 'ok'
      var statisticalData = json.statistical_data;
      // console.log("userinfo", statisticalData)
      var articleData = eval(json.article_data);
